    ...
    async ready() {
        ...
        guild.channels.cache
            ...
            for (const [memberId, _value] of members) {
                ...
                usersTrackingList.push({ userId, discordChannelId: channel.id, status: studentcher_shared_utils_1.Constants.DISCORD_MEMBER_ACTIVE_STATUS });
                loggedInUsers[userId] = true;
            }
        });
        for (const userId in this.usersIndex)
            if (!loggedInUsers[userId])
                usersTrackingList.push({ userId, discordChannelId: null, status: studentcher_shared_utils_1.Constants.DISCORD_MEMBER_LEFT_STATUS });
